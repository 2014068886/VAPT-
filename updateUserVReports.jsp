...
<html>
    ...
    <body>
        ...
        <sql:update dataSource="${dbSource}" var = "update">
            update landbank.vreport set qtrReport=?, vapt_date=?, TCR=?, received_date=?, results=?, remarks=?, date_modified=?, updated_by=? where keyId="${param.id}"
        ...
        <c:if test = "${update>=1}">
            ...
            <%
                ...
                out.println("alert('User Report Successfully Updated')");
                out.println("</script> </head> </html>");
                RequestDispatcher rd = request.getRequestDispatcher("VAPT_UserVReports.jsp");
